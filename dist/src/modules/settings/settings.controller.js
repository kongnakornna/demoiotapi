...
const dashboardConfig_dto_1 = require("./dto/dashboardConfig.dto");
var Cache = new redis_cache_1.CacheDataOne();
var md5 = require('md5');
require("dotenv/config");
var tzString = process.env.tzString;
var SEND_EMAIL = process.env.SEND_EMAIL;
var connectUrl_mqtt = process.env.MQTT_HOST || 'mqtt://172.25.99.10:1883';
if (!connectUrl_mqtt) {
...
let SettingsController = class SettingsController {
    ...
    async list_mqtt(res, query, headers, params, req) {
        ...
        for (const [key, va] of Object.entries(ResultData)) {
            ...
            var date = ResultData[key].updateddate;
            if (date) {
                var date = format.timeConvertermas(format.convertTZ(ResultData[key].updateddate, process.env.tzString));
            }
            const RSdata = {
                mqtt_id: ResultData[key].mqtt_id,
    ...
    async device_listdevicepage(res, query, headers, params, req) {
        ...
        var kaycache = 'mqtt_device_list_page_v2_' + filterkeymd5;
        if (deletecache == 1) {
    ...
    async list_device_page_ensor(res, query, headers, params, req) {
        const device_id = query.device_id || '';
        const page = Number(query.page) || 1;
        const pageSize = Number(query.pageSize) || 1000;
        ...
        const rowResultData = await this.settingsService.device_list_paginate_all(filter);
        if (!rowResultData ||
            rowResultData === '' ||
            rowResultData.status === '422') {
            res.status(200).json({
        ...
        const filter2 = Object.assign(Object.assign({}, filter), { isCount: 0, page,
            pageSize });
        const ResultData = await this.settingsService.device_list_paginate_all(filter2);
        const RssData = [];
        ...
        if (ResultData && Array.isArray(ResultData)) {
            for (const va of ResultData) {
                ...
                if (hardware_id == 1) {
                    ...
                    var typename = 'IO Sensor';
                }
                else if (hardware_id == 3) {
                    var typename = 'IO Control';
                }
                else if (hardware_id == 4) {
                    var typename = 'Critical Sensor';
                }
                else {
                    var typename = 'Sensor';
                }
    ...
    async device_list_paginate_all_active_schedule(res, query, headers, params, req) {
        ...
        var sort = query.sort;
        var keyword = query.keyword || '';
        var hardware_id = query.hardware_id;
        var filter = {
            sort,
            device_id,
            mqtt_id: query.mqtt_id || '',
        ...
        for (var va of ResultData) {
            ...
            const arraydata = {
                ...
                schedule_title: scheduleResultData['0'].schedule_name +
                    ' ' +
    ...
    async device_list_alarm(res, query, headers, params, req) {
        ...
        for (const [key, va] of Object.entries(ResultData)) {
            ...
            var main_overFan1 = mqttdata['payload']['overFan1'];
            var main_overFan2 = mqttdata['payload']['overFan2'];
            if ((main_temperature >= main_status_warning && main_type_id == 1) ||
            ...
            tempData.push(va);
            if ((main_overFan1 != '1' && main_type_id == 2) ||
    ...
    async create_nodered(res, dto, DataDto, query, headers, params, req) {
        ...
        if (Rs) {
            ...
            res.status(200).json({
                ...
                message_th: 'ข้อมูล nodered_name ' +
    ...
    async create_telegram(res, dto, DataDto, query, headers, params, req) {
        ...
        if (Rs) {
            ...
            res.status(200).json({
                ...
                message_th: 'ข้อมูล telegram_name ' +
    ...
    async Deviceactionuser(res, dto, DataDto, query, headers, params, req) {
        ...
        if (Rs) {
            ...
            res.status(200).json({
                ...
                message_th: 'ข้อมูล alarm_action_id ' +
    ...
    async listalarmeventdevicecontrolpage(res, query, headers, params, req) {
        ...
        var eventResultData = await this.settingsService.get_alarm_device_event_map(alarm_action_id);
        var rowResultData = await this.settingsService.device_list_paginate_all_active_control(filter);
        if (rowResultData == '' ||
            !rowResultData ||
            rowResultData.status == '422') {
            res.status(200).json({
        ...
        var alarmData = await this.settingsService.device_alarm_list_paginate(schedule_filter);
        var ResultData = await this.settingsService.device_list_paginate_all_active_control(filter2);
        var tempData2 = [];
        for (var va of ResultData) {
            var device_id = va.device_id;
            var mqtt_id = va.mqtt_id;
                ...
                if (today_status == 1 && now_time_1 == now_time_2) {
                    if (now_time_1 == '1' && now_time_2 == '1') {
                        ...
                        if (log_count >= 1) {
                            ...
                            if (log_count2 == 0) {
                                var deviceData = await this.settingsService.getdevicedata(mqtt_data_value);
                                if (deviceData) {
                                    var devicecontrol = await this.settingsService.devicecontrol(mqtt_data_control, message_mqtt_control);
                                    var now_time_s = timestamp;
                                    var datasetupdate = {
                                        schedule_id: schedule_id,
                                        device_id: device_id,
                            ...
                            if (log_count == 0) {
                                var devicecontrol = await this.settingsService.devicecontrols(mqtt_data_control, message_mqtt_control, message_control);
                            }
                            var now_time_s = timestamp;
                            var deviceData = await this.settingsService.getdevicedata(mqtt_data_value);
                            if (deviceData) {
    ...
    async alarm_to_control_rss1(dto) {
        ...
        var kaycachecache = 'alarm_device_id_event_rss' +
            md5(alarm_action_id + '-' + device_id + '-' + bucket);
        if (deletecache == 1) {
            await Cache.DeleteCacheData(kaycachecache);
        }
    ...
    createdashboardconfig(createDto) {
        var location_id = createDto.location_id;
        return this.settingsService.createDashboardConfig(createDto);
    }
    async dashboardconfig_1(res, query) {
        try {
    ...
    async dashboardconfig(res, query) {
        try {
            if (!query.location_id) {
                return res.status(404).json({
                    statusCode: 404,
            ...
            if (ResultData) {
                for (const [key, item] of Object.entries(ResultData)) {
                    const ConfigRs = {
                        ...
                        status: item.status,
                        updateddate: format.timeConvertermas(format.convertTZ(item.config_data.updated_at, process.env.tzString)),
                    };
                    rtss.push(ConfigRs);
                }
            }
    ...
    findOnedashboardconfig(id) {
        return this.settingsService.findOneDashboardConfig(id);
    }
    async findByCriteria(name, locationId) {
        return await this.settingsService.findOrCreateConfig(name, parseInt(locationId));
    }
    updatedashboardconfig(id, updateDto) {
...
__decorate([
    ...
    __metadata("design:paramtypes", [Object, Object, Object, Object, Object]),
    __metadata("design:returntype", Promise)
], SettingsController.prototype, "list_device_page_ensor", null);
__decorate([
    (0, common_1.HttpCode)(200),
...
__decorate([
    (0, common_1.HttpCode)(200),
    (0, swagger_1.ApiOperation)({ summary: 'Save new dashboard positions' }),
    (0, common_1.Post)('dashboardconfig'),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [dashboardConfig_dto_1.CreateDashboardConfigDto]),
    __metadata("design:returntype", void 0)
], SettingsController.prototype, "createdashboardconfig", null);
__decorate([
    (0, common_1.Get)('dashboardconfig_1'),
    __param(0, (0, common_1.Res)()),
    __param(1, (0, common_1.Query)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, Object]),
    __metadata("design:returntype", Promise)
], SettingsController.prototype, "dashboardconfig_1", null);
__decorate([
    (0, common_1.Get)('dashboardconfig'),
    __param(0, (0, common_1.Res)()),
    __param(1, (0, common_1.Query)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, Object]),
    __metadata("design:returntype", Promise)
], SettingsController.prototype, "dashboardconfig", null);
__decorate([
    (0, common_1.Get)('dashboardconfig/:id'),
    (0, swagger_1.ApiOperation)({ summary: 'Get dashboard config by ID' }),
    __param(0, (0, common_1.Param)('id')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", void 0)
], SettingsController.prototype, "findOnedashboardconfig", null);
__decorate([
    (0, common_1.Get)('dashboardconfig/search'),
    __param(0, (0, common_1.Query)('name')),
    __param(1, (0, common_1.Query)('location_id')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, String]),
    __metadata("design:returntype", Promise)
], SettingsController.prototype, "findByCriteria", null);
__decorate([
    (0, common_1.Patch)('dashboardconfig/:id'),
    (0, swagger_1.ApiOperation)({ summary: 'Update dashboard config positions' }),
    __param(0, (0, common_1.Param)('id')),
    __param(1, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, dashboardConfig_dto_1.UpdateDashboardConfigDto]),
    __metadata("design:returntype", void 0)
], SettingsController.prototype, "updatedashboardconfig", null);
__decorate([
    (0, common_1.Delete)('dashboardconfig/:id'),
    (0, swagger_1.ApiOperation)({ summary: 'Delete dashboard config' }),
    __param(0, (0, common_1.Param)('id')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", void 0)
